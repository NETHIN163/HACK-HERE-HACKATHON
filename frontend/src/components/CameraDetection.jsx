...
export default function CameraDetection() {
  ...
  const [counts, setCounts] = useState({ vehicles: 0, pedestrians: 0, total: 0, labels: [] });
  ...
  const detectFrame = async () => {
    ...
    setCounts({ vehicles, pedestrians, total: predictions.length, labels: [...new Set(predictions.map((prediction) => prediction.class))] });
    animationRef.current = requestAnimationFrame(detectFrame);
  };
  ...
  const stopCamera = () => {
    ...
    setRunning(false);
    setCounts({ vehicles: 0, pedestrians: 0, total: 0, labels: [] });
      ...
      <div className="camera-grid">
        ...
        <div className="camera-controls">
          ...
          <div className="detection-metrics">
            ...
            <div><strong>{counts.total}</strong><span>Total objects</span></div>
          </div>

...
class EmergencyService:
    ...
    def __init__(self, network=None):
        ...
        self.qubo_service = None  # Will be set externally
        self.qaoa_service = None  # Will be set externally
        self.conflict_service = None  # Will be set externally
    ...
    def create_request(self, request_id: str, origin: str, destination: str, priority: EmergencyPriority) -> EmergencyRequest:
        ...
        em = create("TRAUMA", priority_map.get(priority, 3), origin, destination)
    ...
    def register_ambulance(self, vehicle_id: str, current_location: str) -> EmergencyVehicle:
        ...
        if existing:
            ...
            return EmergencyVehicle(vehicle_id=vehicle_id, current_location=current_location, status=AmbulanceStatus.AVAILABLE)
    ...
    def run_qaoa_route_optimization(self, origin: str, destination: str):
        ...
        if self.qaoa_service:
            return self.qaoa_service.run_qaoa_route_optimization(origin, destination)
        # Fallback
        from ..models.qubo import QAOAResult, QUBOFormulation, QUBOVariable
        route = self.calculate_route(origin, destination)
        return QAOAResult(
            selected_route=route,
            selected_variable="x_0",
            qubo_cost=0.0,
            is_valid=True,
    ...
    def activate_corridor(self, request_id: str):
        ...
        return self.plan_corridor(request_id) if False else GreenCorridorPlan(
            corridor_id="temp",
            request_id=request_id,
            vehicle_id="",
            route_id="",
